        ...
        <div className="header-inner">
          ...
          <Link href="/" className="brand-lockup" aria-label="Renova home">
            ...
            <span>
              ...
              <small>Everyday finds, renewed.</small>
            </span>
          </Link>
          <nav
          ...
          <div className="header-actions">
            ...
            <Link href="/saved" className="header-action">
              ♡<span>Saved</span>
            </Link>
            <button
          ...
          >
            <div className="mega-menu-head">
              <div>
                ...
                <h2>Everything, clearly organised.</h2>
              </div>
              <Link
                href="/shop"
                onClick={() => {
                ...
                <section key={category.slug}>
                  ...
                  <div>
                    {category.subcategories?.slice(0, 4).map((subcategory) => (
                      <Link
                        key={subcategory}
                        href={`/shop?category=${category.slug}&q=${encodeURIComponent(subcategory)}`}
                        onClick={() => {
                          setCategoriesOpen(false);
                          setMenuOpen(false);
                        }}
        ...
        {searchOpen && (
          <div className="search-shell">
            <form className="search-bar" action="/search">
              <label htmlFor="site-search">Search Renova</label>
              <input
                id="site-search"
                name="q"
                value={searchQuery}
                ...
                autoFocus
                autoComplete="off"
                placeholder="Try “portable blender” or “school bag”"
              />
    ...
    <footer className="site-footer">
      <div className="footer-story">
        ...
        <div>
          <strong>RENOVA</strong>
          <p>
            Useful finds for everyday life, selected with a spirit of renewal.
          </p>
        </div>
      </div>
      <div className="footer-grid">
        <div>
          ...
          <Link href="/collections/fashion">Fashion</Link>
          <Link href="/collections/electronics">Electronics</Link>
          <Link href="/collections/home-office">Home & office</Link>
        </div>
        <div>
          <h3>Help</h3>
        ...
        <div className="newsletter">
          ...
          <p>
            {BUSINESS_IDENTITY.name} is an{" "}
            {BUSINESS_IDENTITY.legalDescription.toLowerCase()} serving customers
            across Nigeria.
          </p>
      ...
      <div className="footer-bottom">
        <span>© 2026 Renova. Lagos, Nigeria.</span>
        <span>Secure prepaid checkout powered by Paystack.</span>
      </div>
    </footer>
  );
}
...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="product-card">
      <Link href={productHref(product)} className="product-image-wrap">
        <img
      ...
      <div className="product-card-body">
        ...
        {product.reviewCount > 0 ? (
          <div
            className="rating"
            aria-label={`${(product.rating / 10).toFixed(1)} out of 5 from ${product.reviewCount} reviews`}
          >
            <span>★★★★★</span>
            <small>({product.reviewCount})</small>
          </div>
        ) : (
          <div className="rating unrated" aria-label="No reviews yet">
            <small>No reviews yet</small>
          </div>
        )}
        <div className="product-commerce-note">
          <span>Free delivery</span>
          {product.stock > 0 && product.stock <= 5 ? (
            <b>Only {product.stock} left</b>
          ) : (
            <b>{product.stock > 0 ? "In stock" : "Unavailable"}</b>
          )}
        </div>
                ...
                contents: [
                  {
                    id: product.slug,
                    quantity: 1,
                    item_price: product.priceKobo / 100,
                  },
                ],
...
export function ProductGallery({ product }: { product: Product }) {
  const media = Array.from(
    new Set(product.gallery?.length ? product.gallery : [product.imageUrl]),
  );
  ...
  return (
    <div className="product-gallery">
      ...
      >
        {isVideo(selected) ? (
          <video src={selected} controls playsInline preload="metadata" />
        ) : (
          <img
            src={selected}
            alt={`${product.name}, view ${selectedIndex + 1} of ${media.length}`}
            width="900"
            height="900"
            decoding="async"
          />
        ...
        {media.length > 1 && (
          <>
            ...
            <small className="gallery-count" aria-live="polite">
      ...
      <div
        ...
        aria-label={`${media.length} product images and videos`}
      >
        {media.map((url, index) => (
          <button
            key={url}
            ...
            aria-label={`View product media ${index + 1} of ${media.length}`}
  ...
  async function submit(event: React.FormEvent) {
    ...
    setMessage(
      response.ok
        ...
        : (payload.error ?? "Could not submit review."),
    );
    if (response.ok)
  ...
  return (
    <form className="customer-review-form" onSubmit={submit}>
      <div>
        ...
        <p>Reviews are moderated before they appear publicly.</p>
      </div>
      <label>
        Your name
        <input
      ...
      <button className="button espresso" disabled={saving}>
...
export function ProductPurchase({ product }: { product: Product }) {
  ...
  const trackAdd = () =>
    trackMetaEvent("AddToCart", {
      ...
      content_type: "product",
      contents: [
        { id: product.slug, quantity, item_price: product.priceKobo / 100 },
      ],
      value: (product.priceKobo * quantity) / 100,
      currency: "NGN",
      num_items: quantity,
    });
  return (
    <div className="purchase-panel">
      <div className="variant-block">
        ...
        <div className="variant-pills">
          {product.variants.map((item) => (
            <button
              key={item}
              className={variant === item ? "is-selected" : ""}
      ...
      <button
        className="button primary wide purchase-button"
        onClick={() => {
          trackAdd();
      ...
      <button
        className="button espresso wide"
        onClick={() => {
          trackAdd();
          add(product, variant, quantity);
      ...
      >
        {product.paymentMode === "cash_on_delivery" ? "Order now" : "Buy now"}
      </button>
      ...
      <div className="purchase-trust">
        <span>
          ...
          {product.paymentMode === "cash_on_delivery"
            ? "Payment on delivery"
            : "Prepaid secure checkout"}
        </span>
        <span>✓ 7-day return request policy</span>
