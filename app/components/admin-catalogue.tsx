...
const emptyReview = { productSlug: "", reviewerName: "", rating: "5", title: "", body: "", isTestData: true };
...
async function optimizeProductImage(file: File) {
  ...
  return new File([blob], file.name.replace(/\.[^.]+$/, "") + "-optimized.webp", { type: "image/webp", lastModified: Date.now() });
}
...
export function AdminCatalogue({ initialProducts, initialReviews, initialOrders, categories, ownerName }: { initialProducts: Product[]; initialReviews: Review[]; initialOrders: AdminOrder[]; categories: Category[]; ownerName: string }) {
  ...
  const [enhancedGalleryUrls, setEnhancedGalleryUrls] = useState<Record<string, string>>({});
  ...
  async function prepareCover(file: File | null) {
    ...
    try {
      const optimized = await optimizeProductImage(file); setPrimaryFile(optimized); setLocalPreview(URL.createObjectURL(optimized));
      if (aiStudioEnabled) { setStudioStatus("AI Product Studio is improving the cover…"); const url = await enhance(optimized); setEnhancedCoverUrl(url); setLocalPreview(url); setStudioStatus("AI enhancement is ready. Review it before publishing."); }
      else setStudioStatus(`Optimized locally · ${Math.max(1, Math.round(optimized.size / 1024))} KB`);
    } catch (error) { setStudioStatus(error instanceof Error ? `${error.message} The locally optimized image will be used.` : "The locally optimized image will be used."); }
  }
  async function prepareGallery(files: File[]) {
    const prepared: File[] = [];
    for (const file of files) {
      if (file.type.startsWith("video/")) { prepared.push(file); continue; }
      setStudioStatus(`Optimizing ${file.name}…`);
      try {
        const optimized = await optimizeProductImage(file); prepared.push(optimized);
        if (aiStudioEnabled) { setStudioStatus(`AI Product Studio is improving ${file.name}…`); const url = await enhance(optimized); setEnhancedGalleryUrls((current) => ({ ...current, [fileKey(optimized)]: url })); }
      } catch (error) { prepared.push(file); setStudioStatus(error instanceof Error ? error.message : "One image could not be optimized."); }
    }
    addGalleryFiles(prepared); setStudioStatus("Gallery preparation complete. Review each image before publishing.");
  }
  ...
  async function submitProduct(event: FormEvent) {
    ...
    try {
      ...
      else if (primaryFile) { setMessage("Uploading cover image…"); imageUrl = await upload(primaryFile); }
      if (!imageUrl) throw new Error("Choose a cover image for product cards.");
      const uploadedGallery: string[] = [];
      for (const media of galleryFiles) { setMessage(`Uploading ${media.name}…`); uploadedGallery.push(enhancedGalleryUrls[fileKey(media)] || await upload(media)); }
      ...
      setMessage(`${editingId ? "Updated" : "Published"} “${saved.name}” successfully.`); window.setTimeout(() => setProductModalOpen(false), 800);
    } catch (error) { setMessage(error instanceof Error ? error.message : "Something went wrong."); } finally { setSaving(false); }
  }
...
<span className="status-dot"/>Private owner workspace<Link href="/">Open storefront ↗</Link>
</div>
</aside>
...
<span>Add or edit products by department, upload product images or videos, control stock and pricing, and manage presentation or genuine reviews.</span>
</section>
      <section id="overview" className="admin-metrics">
<article>
<span>Published products</span>
<strong>{products.filter((item) => item.isPublished).length}</strong>
...
<div className="admin-category-grid">{categories.map((category) => { const count = products.filter((product) => product.categorySlug === category.slug).length; return <article key={category.slug}>
...
<button onClick={() => openNew(category.slug)}>＋ Add here</button>
<button className="category-view" onClick={() => { setFilter(category.slug); document.getElementById("catalogue")?.scrollIntoView({ behavior: "smooth" }); }}>Manage</button>
</article>; })}</div>
</section>
...
<h2>{filter === "all" ? "All products" : categoryName(filter)}</h2>
</div>
<div className="catalogue-tools">
<input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search products or SKU"/>
<select value={filter} onChange={(event) => setFilter(event.target.value)}>
<option value="all">All categories</option>{categories.map((category) => <option value={category.slug} key={category.slug}>{category.name}</option>)}</select>
<button onClick={() => openNew(filter === "all" ? undefined : filter)}>Add product</button>
</div>
</div>
...
<span>Status</span>
<span>Actions</span>
</div>{visibleProducts.map((product) => <div className="table-row" key={product.slug}>
<span className="admin-product-name">
<img src={product.imageUrl} alt=""/>
<span>
<b>{product.name}</b>
<small>{product.sku} · {product.gallery?.length ?? 1} media</small>
</span>
</span>
...
<span>
<i className={product.isPublished ? "published" : "draft"}>{product.isPublished ? "Published" : "Draft"}</i>
</span>
...
<i>{review.isTestData ? "Presentation sample" : "Customer review"}</i>
</div>
<h3>{review.title}</h3>
<p>{review.body}</p>
<footer>
...
<p>Verified Paystack orders will appear here automatically.</p>
    ...
    {productModalOpen && <div className="product-modal" role="dialog" aria-modal="true">
<button className="modal-scrim" onClick={() => !saving && setProductModalOpen(false)} aria-label="Close"/>
...
<button type="button" className="icon-button" onClick={() => setProductModalOpen(false)}>×</button>
</header>
<div className="form-layout">
<div className="form-primary">
      <fieldset>
<legend>1. Product details</legend>
<label>Product name<input required value={form.name} onChange={(e) => updateField("name", e.target.value)}/>
</label>
<div className="form-two">
<label>Category<select value={form.categorySlug} onChange={(e) => updateField("categorySlug", e.target.value)}>{categories.map((category) => <option value={category.slug} key={category.slug}>{category.name}</option>)}</select>
</label>
<label>SKU <small>Optional for new products</small>
<input value={form.sku} onChange={(e) => updateField("sku", e.target.value)} placeholder="Auto-generated if blank"/>
</label>
</div>
<label>Short sales description<input required value={form.shortDescription} onChange={(e) => updateField("shortDescription", e.target.value)}/>
</label>
<label>Full description<textarea rows={6} value={form.description} onChange={(e) => updateField("description", e.target.value)} placeholder="Benefits, specifications, dimensions and what is included"/>
</label>
</fieldset>
      <fieldset>
<legend>2. Specifications customers can trust</legend>
<div className="form-two"><label>Brand<input value={form.brand} onChange={(e) => updateField("brand", e.target.value)}/></label><label>Model<input value={form.model} onChange={(e) => updateField("model", e.target.value)}/></label><label>Materials<input value={form.materials} onChange={(e) => updateField("materials", e.target.value)}/></label><label>Dimensions<input value={form.dimensions} onChange={(e) => updateField("dimensions", e.target.value)}/></label><label>Weight<input value={form.weight} onChange={(e) => updateField("weight", e.target.value)}/></label><label>Colours<input value={form.colour} onChange={(e) => updateField("colour", e.target.value)}/></label><label>Size<input value={form.size} onChange={(e) => updateField("size", e.target.value)}/></label><label>Warranty<input value={form.warranty} onChange={(e) => updateField("warranty", e.target.value)}/></label><label>Country of origin<input value={form.countryOfOrigin} onChange={(e) => updateField("countryOfOrigin", e.target.value)}/></label><label>Compatibility<input value={form.compatibility} onChange={(e) => updateField("compatibility", e.target.value)}/></label></div>
<label>What is in the box<textarea rows={3} value={form.packageContents} onChange={(e) => updateField("packageContents", e.target.value)}/></label>
<label>Care instructions<textarea rows={3} value={form.careInstructions} onChange={(e) => updateField("careInstructions", e.target.value)}/></label>
<label>Additional specifications <small>One “Label: Value” per line</small><textarea rows={5} value={form.specifications} onChange={(e) => updateField("specifications", e.target.value)} placeholder={'Battery: 5,000 mAh\nDisplay: 6.5 inch'}/></label>
</fieldset>
      <fieldset className="assistant-fields">
<legend>3. Product assistant knowledge</legend><p>Only add approved customer-facing facts. Private supplier details are never sent to the AI provider.</p>
<label>Knowledge notes<textarea rows={7} value={form.chatbotKnowledge} onChange={(e) => updateField("chatbotKnowledge", e.target.value)} placeholder="Explain use cases, setup, delivery expectations, limitations and approved answers."/></label>
<label>Frequently asked questions <small>One “Question | Answer” per line</small><textarea rows={6} value={form.chatbotFaq} onChange={(e) => updateField("chatbotFaq", e.target.value)} placeholder={'Is it rechargeable? | Yes, using the included USB-C cable.\nWhen will it arrive? | Usually within 3–5 working days.'}/></label>
</fieldset>
      <fieldset>
<legend>4. Pricing, discount and stock</legend>
<div className="form-three">
<label>Current selling price (₦)<input required type="number" min="0" value={form.priceNaira} onChange={(e) => updateField("priceNaira", e.target.value)}/>
</label>
<label>Previous / compare price (₦)<input type="number" min="0" value={form.compareAtNaira} onChange={(e) => updateField("compareAtNaira", e.target.value)}/>
...
<input value={form.variants} onChange={(e) => updateField("variants", e.target.value)}/>
</label>
<label>Promotion label <small>Optional</small>
...
<p>These fields stay in the owner database and never appear on the storefront.</p>
<div className="form-two">
<label>Supplier name<input value={form.supplierName} onChange={(e) => updateField("supplierName", e.target.value)}/>
</label>
<label>Supplier cost (₦)<input type="number" min="0" value={form.supplierCostNaira} onChange={(e) => updateField("supplierCostNaira", e.target.value)}/>
</label>
</div>
<label>Supplier product URL<input type="url" value={form.supplierUrl} onChange={(e) => updateField("supplierUrl", e.target.value)}/>
</label>
</fieldset>
    </div>
...
<input required={!form.imageUrl} type="file" accept="image/jpeg,image/png,image/webp,image/avif" onChange={(e) => void prepareCover(e.target.files?.[0] ?? null)}/>{localPreview ? <img src={localPreview} alt="Product cover preview"/> : <>
<b>＋</b>
<strong>Choose cover image</strong>
<span>Used on product cards and social previews.</span>
</>}</label>
{originalCoverPreview && <div className="ai-before-after"><article><span>Original</span><img src={originalCoverPreview} alt="Original uploaded product"/></article><article><span>{enhancedCoverUrl ? "AI enhanced" : "Optimized"}</span><img src={localPreview} alt="Prepared product"/></article></div>}
...
<strong>{form.priceNaira ? `₦${Number(form.priceNaira).toLocaleString("en-NG")}` : "₦0"}</strong>
<small>{form.compareAtNaira ? `Previous price ₦${Number(form.compareAtNaira).toLocaleString("en-NG")}` : "No compare-at price"}</small>
</div>
</aside>
</div>
<footer>
<span className={message.toLowerCase().includes("failed") || message.toLowerCase().includes("required") ? "error-message" : "save-message"}>{message}</span>
<button type="button" className="button quiet" onClick={() => setProductModalOpen(false)}>Cancel</button>
<button type="submit" disabled={saving} className="button primary">{saving ? "Working…" : editingId ? "Save changes" : form.isPublished ? "Publish product" : "Save draft"}</button>
</footer>
</form>
</div>}

    {reviewModalOpen && <div className="product-modal" role="dialog" aria-modal="true">
<button className="modal-scrim" onClick={() => setReviewModalOpen(false)} aria-label="Close"/>
<form className="review-form" onSubmit={submitReview}>
<header>
<div>
<span className="eyebrow">Review manager</span>
<h2>Add a product review</h2>
<p>Use “presentation sample” for pitch content. Import genuine feedback only with permission.</p>
</div>
...
<label className="check-row">
<input type="checkbox" checked={reviewForm.isTestData} onChange={(e) => setReviewForm((current) => ({ ...current, isTestData: e.target.checked }))}/>
<span>
<b>Presentation sample</b>
<small>Labels demonstration content on the storefront.</small>
</span>
</label>{reviewMessage && <p className="save-message">{reviewMessage}</p>}</div>
<footer>
