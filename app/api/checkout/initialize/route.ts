...
export async function POST(request: Request) {
  try {
    ...
    if (!secret) return Response.json({ error: "Paystack checkout is waiting for the store owner’s secret key." }, { status: 503 });
    ...
    for (const line of payload.lines) {
      ...
      verifiedItems.push({ slug: product.slug, name: product.name, sku: product.sku, categorySlug: product.categorySlug, variant: String(line.variant ?? product.variants[0] ?? "Standard"), quantity, unitPriceKobo: product.priceKobo, supplierCostKobo: product.supplierCostKobo ?? null, imageUrl: product.imageUrl });
    }
    ...
    const origin = process.env.APP_URL?.trim().replace(/\/$/, "") || new URL(request.url).origin;
    const paystackResponse = await fetch("https://api.paystack.co/transaction/initialize", {
      method: "POST",
      headers: { authorization: `Bearer ${secret}`, "content-type": "application/json" },
      body: JSON.stringify({ email: details.email, amount: totalKobo, currency: "NGN", reference: orderNumber, callback_url: `${origin}/api/paystack/verify?reference=${encodeURIComponent(orderNumber)}`, metadata: { order_number: orderNumber, customer_name: details.fullName, cancel_action: `${origin}/checkout?payment=cancelled` } }),
    });
