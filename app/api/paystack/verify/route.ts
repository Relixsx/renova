...
export async function GET(request: Request) {
  ...
  try {
    ...
    const payload = await response.json() as { status?: boolean; data?: { status?: string; amount?: number; reference?: string; currency?: string } };
    if (!response.ok || !payload.status || payload.data?.status !== "success" || payload.data?.reference !== reference || payload.data?.currency !== "NGN") return Response.redirect(`${url.origin}/checkout?payment=failed`, 303);
