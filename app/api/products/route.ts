...
const detailFields = ["brand", "model", "materials", "dimensions", "weight", "colour", "size", "warranty", "packageContents", "countryOfOrigin", "careInstructions", "compatibility", "chatbotKnowledge"] as const;
function textValue(payload: Record<string, unknown>, key: typeof detailFields[number]) { return String(payload[key] ?? "").trim(); }
function parseSpecifications(value: unknown) {
  if (value && typeof value === "object" && !Array.isArray(value)) return value as Record<string, string>;
  return Object.fromEntries(String(value ?? "").split("\n").map((line) => line.split(":")).filter((parts) => parts.length > 1).map(([key, ...rest]) => [key.trim(), rest.join(":").trim()]).filter(([key, value]) => key && value));
}
function parseFaq(value: unknown) {
  if (Array.isArray(value)) return value;
  return String(value ?? "").split("\n").map((line) => line.split("|")).filter((parts) => parts.length > 1).map(([question, ...answer]) => ({ question: question.trim(), answer: answer.join("|").trim() })).filter((item) => item.question && item.answer);
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const compareAtKobo = payload.compareAtNaira ? asKobo(payload.compareAtNaira) : null;
    ...
    if (!name || !categorySlug || priceKobo === null || !imageUrl) {
      return Response.json({ error: "Product name, category, selling price and image are required." }, { status: 400 });
    }
    ...
    const sku = String(payload.sku ?? "").trim() || `REN-${crypto.randomUUID().slice(0, 8).toUpperCase()}`;
    const [created] = await db.insert(products).values({
      ...
      supplierCostKobo,
      imageUrl,
      galleryJson: JSON.stringify(Array.isArray(payload.gallery) && payload.gallery.length ? payload.gallery : [imageUrl]),
      variantsJson: JSON.stringify(variants.length ? variants : ["Standard"]),
      specificationsJson: JSON.stringify(parseSpecifications(payload.specifications)),
    ...
    return Response.json({ error: error instanceof Error ? error.message : "Could not create the product." }, { status: 500 });
  }
}
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (payload.categorySlug !== undefined) updates.categorySlug = String(payload.categorySlug).trim();
    if (payload.shortDescription !== undefined) updates.shortDescription = String(payload.shortDescription).trim();
    ...
    if (payload.priceNaira !== undefined) {
      ...
      if (value === null) return Response.json({ error: "Enter a valid selling price." }, { status: 400 });
      updates.priceKobo = value;
    }
    if (payload.compareAtNaira !== undefined) updates.compareAtKobo = payload.compareAtNaira ? asKobo(payload.compareAtNaira) : null;
    if (payload.supplierCostNaira !== undefined) updates.supplierCostKobo = payload.supplierCostNaira ? asKobo(payload.supplierCostNaira) : null;
    if (payload.imageUrl !== undefined) updates.imageUrl = String(payload.imageUrl).trim();
    if (payload.gallery !== undefined) updates.galleryJson = JSON.stringify(Array.isArray(payload.gallery) ? payload.gallery : []);
    if (payload.variants !== undefined) updates.variantsJson = JSON.stringify(Array.isArray(payload.variants) ? payload.variants : []);
    if (payload.specifications !== undefined) updates.specificationsJson = JSON.stringify(parseSpecifications(payload.specifications));
    ...
    if (typeof payload.isPublished === "boolean") updates.isPublished = payload.isPublished;
    if (typeof payload.isFeatured === "boolean") updates.isFeatured = payload.isFeatured;
    const [updated] = await db.update(products).set(updates).where(eq(products.id, payload.id)).returning();
