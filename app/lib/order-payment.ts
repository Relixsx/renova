...
async function notifyCustomer(order: typeof orders.$inferSelect) {
  ...
  const html = `<div style="font-family:Arial,sans-serif;color:#2b1812;max-width:620px;margin:auto"><h1 style="color:#e85b3f">Payment confirmed</h1><p>Hello ${escapeHtml(order.customerName)}, your Renova order has been received and payment verified.</p><h2>Order ${escapeHtml(order.orderNumber)}</h2><ul>${lines}</ul><p><b>Amount paid:</b> ₦${(order.totalKobo / 100).toLocaleString("en-NG")}</p><p><b>Delivery address:</b> ${[address.streetAddress, address.town, address.localGovernment, address.state].filter(Boolean).map(escapeHtml).join(", ")}</p><p><b>Carrier:</b> ${escapeHtml(shipping.name ?? shipping.carrier ?? "Renova delivery partner")}<br><b>Estimated delivery:</b> ${escapeHtml(order.estimatedDelivery ?? "3–5 working days")}</p><p>Track the order using <b>${escapeHtml(order.orderNumber)}</b> and this email address on the Renova tracking page.</p><p>Delivery instruction: ${escapeHtml(address.deliveryInstruction ?? "None provided")}</p><p>Questions? Email <a href="mailto:support@shoprenova.com.ng">support@shoprenova.com.ng</a>.</p></div>`;
